from vpython import scene, slider, wtext
...
class View():
    def __init__(self, Simulation):
        ...
        speedSlider = slider(bind=self.manipulateSimulationSpeed, max=MAX_SIM_RATE, min=MIN_SIM_RATE, value=INITIAL_SIM_RATE, id='simSpeed', step=RATE_STEP_SLIDER)
        self.wt = wtext(text='{:d}'.format(speedSlider.value) + ' Simulation Steps per Second')
        scene.append_to_caption('\nElasticity factor:   ')
        elasticitySlider = slider(bind=self.manipulateSimulationSpeed, max=MAX_ELASTICITY, min=MIN_ELASTICITY, value=INITIAL_ELASTICITY, id='elasticity', step=ELASTICITY_STEP_SLIDER)
        self.vt = wtext(text='{:1.2f}'.format(elasticitySlider.value))

    def manipulateSimulationSpeed(self,event):
